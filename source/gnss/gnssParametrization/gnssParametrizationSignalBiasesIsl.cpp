...
GnssParametrizationSignalBiasesIsl::GnssParametrizationSignalBiasesIsl(Config &config)
{
  try
  {
    ...
    readConfig(config, "selectTransmitIslTerminal",       selectTransmitters,     Config::DEFAULT,  R"(["all"])", "");
    readConfig(config, "selectReceiveIslTerminal",        selectReceivers,        Config::DEFAULT,  R"(["all"])", "");
    ...
    if(!fileNameInTransmitter.empty())
    {
      ...
      for(UInt idTrans=0; idTrans<gnss->transmitters.size(); idTrans++)
        if(selectedTransmitters.at(idTrans) && gnss->transmitters.at(idTrans)->useable())
        {
          ...
          try
          {
            readFileIslSignalBias(fileNameInTransmitter(fileNameVariableList), gnss->transmitters.at(idTrans)->signalBiasIslTx);
          }
          catch(std::exception &/*e*/)
          {
    ...
    if(!fileNameInReceiver.empty())
    {
      ...
      for(UInt idTrans=0; idTrans<gnss->transmitters.size(); idTrans++)
        if(selectedTransmitters.at(idTrans) && gnss->transmitters.at(idTrans)->useable())
        {
          ...
          try
          {
            readFileIslSignalBias(fileNameInReceiver(fileNameVariableList), gnss->transmitters.at(idTrans)->signalBiasIslRx);
          }
          catch(std::exception &/*e*/)
          {
  ...
  try
  {
    ...
    if(!fileNameOutTransmitter.empty() && Parallel::isMaster(normalEquationInfo.comm))
    {
      ...
      for(auto trans : gnss->transmitters)
        ...
        {
          IslSignalBias signalBias = trans->signalBiasIslTx;
    ...
    if(!fileNameOutReceiver.empty() && Parallel::isMaster(normalEquationInfo.comm))
    {
      ...
      for(auto trans : gnss->transmitters)
        if(trans->useable() && selectedTransmitters.at(trans->idTrans()))
        {
          IslSignalBias signalBias = trans->signalBiasIslRx;
          fileNameVariableList.setVariable("prn", trans->name());
