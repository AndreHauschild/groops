...
\configClass{selectReceivers}{platformSelectorType} for GNSS observations and all
\configClass{selectTransmitters}{platformSelectorType} for ISL observations.
...
inline GnssProcessingStepWriteResiduals::GnssProcessingStepWriteResiduals(Config &config)
{
  try
  {
    ...
    readConfig(config, "selectTransmitter",       selectTransmitters,   Config::MUSTSET, "", "subset of used transmitters");
    readConfig(config, "outputfileResidualsIsl",  fileNameResidualsIsl, Config::MUSTSET, "output/residuals_{loopTime:%D}.{prn}.dat", "variable {prn} available");
  }
  ...
  try
  {
    ...
    for(auto recv : state.gnss->transmitters)
      if(selectedTransmitters.at(recv->idTrans()))
      {
        ...
        InstrumentFile::write(fileNameResidualsIsl(fileNameVariableList), arc);
      } // for(trans)
  }
  catch(std::exception &e)
  {
