...
GROOPS_REGISTER_PROGRAM(GnssSimulateIslOld, PARALLEL, "GNSS ISL simulation", Gnss, Simulation)

/***********************************************/

void GnssSimulateIslOld::run(Config &config, Parallel::CommunicatorPtr comm)
{
  try
  {
...
void GnssSimulateIslOld::readFile(const FileName &fileName, const std::vector<Time> &times, const Time &timeMargin, GnssReceiverArc &scheduleIsl) const
{
  try
  {
